...
public class EquipoController {
    ...
    // CAMBIO APLICADO: @RequestParam y @RequestHeader actualizados a "idUsuario"
    public ResponseEntity<?> inscribirIntegrante(@PathVariable Long equipoId,
                                                 @RequestParam("idUsuario") Long idUsuario,
                                                 @RequestHeader("idUsuario") Long ejecutorId) {
        String respuesta = equipoService.inscribirIntegrante(equipoId, idUsuario, ejecutorId);
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'ARBITRO')")
    @PutMapping("/{equipoId}")
    public ResponseEntity<EquipoResponseDTO> actualizar(@PathVariable Long equipoId,
